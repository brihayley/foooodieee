 ...
 */
	if ( ! class_exists( 'eminent_Plugin_Activation' ) ) {
    /*  Automatic plugin installation and activation library. */
    class eminent_Plugin_Activation {
        ...
        public $menu = 'eminent-install-plugins';
        ...
        public function __construct() {
            ...
            do_action_ref_array( 'eminent_init', array( $this ) );
        ...
        public function init() {
            // Load class strings.
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'eminent' ),
                'menu_title'                     => __( 'Install Plugins', 'eminent' ),
                'installing'                     => __( 'Installing Plugin: %s', 'eminent' ),
                'oops'                           => __( 'Something went wrong.', 'eminent' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'eminent' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'eminent' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'eminent' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'eminent' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'eminent' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'eminent' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'eminent' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'eminent' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'eminent' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'eminent' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'eminent' ),
                'dashboard'                      => __( 'Return to the dashboard', 'eminent' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'eminent' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'eminent' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'eminent' ),
                'dismiss'                        => __( 'Dismiss this notice', 'eminent' ),
            );
            do_action( 'eminent_register' );
        ...
        public function thickbox() {
            if ( ! get_user_meta( get_current_user_id(), 'eminent_dismissed_notice_' . $this->id, true ) ) {
                add_thickbox();
            } }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new eminentPA_List_Table;
            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'eminent-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            } ?>

<div class="eminent wrap">
  <h2><?php _e(esc_html( get_admin_page_title() ), 'eminent'); ?></h2>
  <?php $plugin_table->prepare_items(); ?>
  <?php if ( isset( $this->message ) ) {
                    echo( wp_kses_post( $this->message ));
                } ?>
  <form id="eminent-plugins" action="" method="post">
    <input type="hidden" name="eminent-page" value="<?php echo $this->menu; ?>" />
...
<?php
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['eminent-install'] ) && 'install-plugin' == $_GET['eminent-install'] ) ) {
                check_admin_referer( 'eminent-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'eminent-install' => 'install-plugin',
                        ),
                ...
                $fields = array( 'eminent-install' ); // Extra fields to pass to WP_Filesystem.
			...
			echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . $this->strings['return'] . '</a></p>';
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'eminent' ) . '">' . __( 'Return to the Dashboard', 'eminent' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                } return true;
            }
            // Checks for actions from hover links to process the activation.
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['eminent-activate'] ) && 'activate-plugin' == $_GET['eminent-activate'] ) ) {
                check_admin_referer( 'eminent-activate', 'eminent-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( $this->is_eminent_page() ) { return; }
            // Return early if the nag message has been dismissed.
            if ( get_user_meta( get_current_user_id(), 'eminent_dismissed_notice_' . $this->id, true ) ) {  return; }
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'eminent' ), $imploded, $count ) . '</p>';
                }
                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'eminent' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'eminent' ) . '</a>'  : '';
                // Define all of the action links.
                $action_links = apply_filters(
                    'eminent_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'eminent-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'eminent', 'eminent', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'eminent', 'eminent', $rendered, $nag_class );
                } }
            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'eminent' );
            } }
        /* Add dismissable admin notices. */
        public function dismiss() {
            if ( isset( $_GET['eminent-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'eminent_dismissed_notice_' . $this->id, 1 );
            } }
        ...
        public function actions( $install_actions ) {
            // Remove action links on the eminentPA install page.
            if ( $this->is_eminent_page() ) { return false; }
        ...
        /* Determine if we're on the eminentPA Install page. */
        protected function is_eminent_page() {
        ...
        public function update_dismiss() {
            delete_user_meta( get_current_user_id(), 'eminent_dismissed_notice_' . $this->id );
        }
        ...
        public static function get_instance() {
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof eminent_Plugin_Activation ) ) {
                self::$instance = new eminent_Plugin_Activation();
            } return self::$instance;
        } }
    // Ensure only one instance of the class is ever invoked.
    $eminent = eminent_Plugin_Activation::get_instance();
}
if ( ! function_exists( 'eminent' ) ) {
    /* Helper function to register a collection of required plugins. */
    function eminent( $plugins, $config = array() ) {
        foreach ( $plugins as $plugin ) {
            eminent_Plugin_Activation::$instance->register( $plugin );
        } if ( $config ) {
            eminent_Plugin_Activation::$instance->config( $config );
        } } }
...
if ( ! class_exists( 'eminentPA_List_Table' ) ) {
    /* List table class for handling plugins. */
    class eminentPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            eminent_Plugin_Activation::$instance->admin_init();
            eminent_Plugin_Activation::$instance->thickbox();
            // Prep variables for use and grab list of all installed plugins.
            $table_data        = array();
            $i                 = 0;
            $installed_plugins = get_plugins();
            foreach ( eminent_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'eminent' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'eminent' );
                    } } else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'eminent' );
                }
                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'eminent' ) : __( 'Recommended', 'eminent' );
                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'eminent' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'eminent' ) );
                }
        ...
        /* Retrieve plugin data, given the plugin name. Taken from the eminent_Plugin_Activation class.*/
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            foreach ( eminent_Plugin_Activation::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                } } return false;
        }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'eminent' ) . ' %2$s">' . __( 'Install', 'eminent' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
                                    'page'          => eminent_Plugin_Activation::$instance->menu,
                                    'plugin'        => $item['slug'],
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'eminent' ) . ' %2$s">' . __( 'Activate', 'eminent' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => eminent_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'eminent-activate'       => 'activate-plugin',
                                'eminent-activate-nonce' => wp_create_nonce( 'eminent-activate' ),
                            ), admin_url( 'themes.php' )
                        ), $item['sanitized_plugin']
                    ), );
            }
    ...
    {
      _e('No plugins to install or activate','eminent'); echo ".<a href='admin_url()'>Return to the Dashboard</a>";
      echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
    }
        ...
        public function get_columns() {
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'eminent' ),
                'source' => __( 'Source', 'eminent' ),
                'type'   => __( 'Type', 'eminent' ),
                'status' => __( 'Status', 'eminent' )
            ); return $columns;
        }
        /* Defines all types of bulk actions for handling*/
        public function get_bulk_actions() {
            $actions = array(
                'eminent-bulk-install'  => __( 'Install', 'eminent' ),
                'eminent-bulk-activate' => __( 'Activate', 'eminent' ),
            );
            return $actions;
        }
        ...
        public function process_bulk_actions() {
            ...
            if ( 'eminent-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'eminent' ) . '</p></div>';
                    return false;
                }
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => eminent_Plugin_Activation::$instance->menu,
                            'eminent-action'  => 'install-selected',
                ...
                $sources      = array();
                $install_path = array();
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'eminent-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'eminent-empty';
                    $i++;
                }
                if ( is_wp_error( $api ) ) { wp_die( eminent_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) ); }
                ...
                $url   = add_query_arg( array( 'page' => eminent_Plugin_Activation::$instance->menu ), admin_url( 'themes.php' ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
                // Create a new instance of eminent_Bulk_Installer.
                $installer = new eminent_Bulk_Installer( $skin = new eminent_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );
                // Wrap the install process with the appropriate HTML.
                echo '<div class="eminent wrap">';
                    if ( version_compare( eminent_Plugin_Activation::$instance->wp_version, '3.8', '<' ) ) {
                    }
            ...
            if ( 'eminent-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be activated at this time.', 'eminent' ) . '</p></div>';
                    return false;
                }
                $plugins      = array();
                $plugin_names = array();
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'eminent' ), $imploded );
                }
 ...
 */
add_action( 'admin_init', 'eminent_load_bulk_installer' );
function eminent_load_bulk_installer() {
    if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && eminent_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
        require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
        if ( ! class_exists( 'eminent_Bulk_Installer' ) ) {
            /* Installer class to handle bulk plugin installations. */
            class eminent_Bulk_Installer extends WP_Upgrader {
                ...
                public function bulk_install( $packages ) {
                    ...
                    if ( eminent_Plugin_Activation::$instance->is_automatic ) { $this->activate_strings(); }
                ...
                public function run( $options ) {
                    ...
                    if ( eminent_Plugin_Activation::$instance->is_automatic ) {
                        // Flush plugins cache so we can make sure that the installed plugins list is always up to date.
                        wp_cache_flush();
                        ...
                        eminent_Plugin_Activation::$instance->populate_file_path();
                        // Set correct strings based on results.
                        if ( is_wp_error( $activate ) ) {
                ...
                public function install_strings() {
                    $this->strings['no_package']          = __( 'Install package not available.', 'eminent' );
                    $this->strings['downloading_package'] = printf( __('Downloading install package from %1$s %s %1$s', 'eminent'), '<span class="code">', '</span>&#8230;');
                    $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'eminent' );
                    $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'eminent' );
                    $this->strings['process_failed']      = __( 'Plugin install failed.', 'eminent' );
                    $this->strings['process_success']     = __( 'Plugin installed successfully.', 'eminent' );
                }
                /* Sets the correct activation strings for the installer skin to use.*/
                public function activate_strings() {
                    $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'eminent' );
                    $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'eminent' );
                }
        ...
        if ( ! class_exists( 'eminent_Bulk_Installer_Skin' ) ) {
            /* Installer skin to set strings for the bulk plugin installations. */
            class eminent_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                ...
                public function add_strings() {
                    ...
                    if ( eminent_Plugin_Activation::$instance->is_automatic ) {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'eminent' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'eminent' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'eminent' ) . '</span><span class="hidden">' . __( 'Hide Details', 'eminent' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'eminent' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'eminent' );
                    }
                    // Default installation strings.
                    else {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'eminent' );
                        $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: %2$s.', 'eminent' );
                        $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'eminent' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'eminent' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'eminent' ) . '</span><span class="hidden">' . __( 'Hide Details', 'eminent' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'eminent' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'eminent' );
                    }
                }
                ...
                public function bulk_footer() {
                    ...
                    foreach ( eminent_Plugin_Activation::$instance->plugins as $plugin ) {
                        if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                            echo '<p><a href="' . add_query_arg( 'page', eminent_Plugin_Activation::$instance->menu, admin_url( 'themes.php' ) ) . '" title="' . esc_attr( eminent_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . eminent_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                            $complete[] = $plugin;
                            break;
                        } else {
                            $complete[] = '';
                    ...
                    if ( empty( $complete ) ) {
                        echo '<p>' .  sprintf( eminent_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'eminent' ) . '">' . __( 'Return to the Dashboard', 'eminent' ) . '</a>' ) . '</p>';
                        echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                    } }
                /* Flush header output buffer. */
